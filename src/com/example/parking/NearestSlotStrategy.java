...
public class NearestSlotStrategy implements SlotAssignmentStrategy {
    ...
    @Override
    public Optional<ParkingSlot> findSlot(List<ParkingSlot> slots, VehicleType vehicleType, SlotType requestedSlotType, Gate entryGate) {
        ...
        return slots.stream().filter(s -> !s.isOccupied()).filter(s -> allowed.contains(s.getType())).min((a, b) -> compareByDistance(a, b, entryGate));
    }
